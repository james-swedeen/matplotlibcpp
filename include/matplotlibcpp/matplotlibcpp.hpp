...
//static std::string s_backend = "GTK4Cairo";
...
#  endif
    ...
    _interpreter() {
        ...
        if (!s_backend.empty()) {
          PyObject* res = PyObject_CallMethod(matplotlib, const_cast<char*>("use"), const_cast<char*>("s"), s_backend.c_str());
...
template<typename Numeric, Eigen::Index SIZE = Eigen::Dynamic>
bool plot(const Eigen::Ref<const Eigen::Matrix<Numeric,SIZE,1>>& x, const Eigen::Ref<const Eigen::Matrix<Numeric,SIZE,1>>& y, const std::map<std::string, std::string>& keywords)
{
  const Eigen::Index size = x.rows();
  assert(size == y.rows());
  std::vector<Numeric> x_vec(size);
  std::vector<Numeric> y_vec(size);
  for(Eigen::Index i = 0; i < size; ++i)
  {
    x_vec[i] = x[i];
    y_vec[i] = y[i];
  }
  return plot<Numeric>(x_vec, y_vec, keywords);
}
template<typename Numeric>
...
template<typename NumericX, typename NumericY, Eigen::Index SIZE = Eigen::Dynamic>
bool plot(const Eigen::Ref<const Eigen::Matrix<NumericX,SIZE,1>>& x, const Eigen::Ref<const Eigen::Matrix<NumericY,SIZE,1>>& y, const std::map<std::string, std::string>& keywords)
{
  const Eigen::Index size = x.rows();
  assert(size == y.rows());
  std::vector<NumericX> x_vec(size);
  std::vector<NumericY> y_vec(size);
  for(Eigen::Index i = 0; i < size; ++i)
  {
    x_vec[i] = x[i];
    y_vec[i] = y[i];
  }
  return plot<NumericX,NumericY>(x_vec, y_vec, keywords);
}
template<typename NumericX, typename NumericY>
...
template<typename NumericX, typename NumericY, Eigen::Index SIZE = Eigen::Dynamic>
bool named_plot(const std::string& name, const Eigen::Ref<const Eigen::Matrix<NumericX,SIZE,1>>& x, const Eigen::Ref<const Eigen::Matrix<NumericY,SIZE,1>>& y, const std::string& format = "")
{
  const Eigen::Index size = x.rows();
  assert(size == y.rows());
  std::vector<NumericX> x_vec(size);
  ...
  return named_plot<NumericX,NumericY>(name, x_vec, y_vec, format);
}
...
#define MATPLOTLIBCPP(prefix, Numeric) \
prefix bool matplotlibcpp::plot<Numeric>(const std::vector<Numeric>&,const std::vector<Numeric>&,const std::map<std::string,std::string>&); \
prefix void matplotlibcpp::plot_surface<Numeric>(const std::vector<::std::vector<Numeric>>&,const std::vector<::std::vector<Numeric>>&,const std::vector<::std::vector<Numeric>>&,const std::map<std::string,std::string>&,const long); \
prefix void matplotlibcpp::contour<Numeric>(const std::vector<::std::vector<Numeric>>&,const std::vector<::std::vector<Numeric>>&,const std::vector<::std::vector<Numeric>>&,const std::map<std::string, std::string>&); \
prefix void matplotlibcpp::spy<Numeric>(const std::vector<::std::vector<Numeric>>&,const double,const std::map<std::string,std::string>&); \
prefix void matplotlibcpp::plot3<Numeric>(const std::vector<Numeric>&,const std::vector<Numeric>&,const std::vector<Numeric>&,const std::map<std::string,std::string>&,const long); \
prefix bool matplotlibcpp::stem<Numeric>(const std::vector<Numeric>&,const std::vector<Numeric>&,const std::map<std::string,std::string>&); \
prefix bool matplotlibcpp::fill<Numeric>(const std::vector<Numeric>&,const std::vector<Numeric>&,const std::map<std::string,std::string>&); \
prefix bool matplotlibcpp::fill_between<Numeric>(const std::vector<Numeric>&,const std::vector<Numeric>&, const std::vector<Numeric>& y2, const std::map<std::string, std::string>& keywords); \
prefix bool matplotlibcpp::arrow<Numeric>(Numeric x, Numeric y, Numeric end_x, Numeric end_y, const std::string& fc = "r",const std::string ec = "k", Numeric head_length = 0.25, Numeric head_width = 0.1625); \
prefix bool matplotlibcpp::hist<Numeric>(const std::vector<Numeric>& y, long bins=10,std::string color="b", double alpha=1.0, bool cumulative=false); \
prefix bool matplotlibcpp::scatter<Numeric,Numeric>(const std::vector<Numeric>& x,const std::vector<Numeric>& y,const double s=1.0, const std::map<std::string, std::string> & keywords = {}); \
prefix bool matplotlibcpp::scatter_colored<Numeric,Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y,const std::vector<Numeric>& colors,const double s=1.0, const std::map<std::string, std::string> & keywords = {}); \
prefix bool matplotlibcpp::scatter<Numeric,Numeric,Numeric>(const std::vector<Numeric>& x,const std::vector<Numeric>& y,const std::vector<Numeric>& z,const double s=1.0,const std::map<std::string, std::string> & keywords = {},const long fig_number=0); \
prefix bool matplotlibcpp::boxplot<Numeric>(const std::vector<std::vector<Numeric>>& data,const std::vector<std::string>& labels = {},const std::map<std::string, std::string> & keywords = {}); \
prefix bool matplotlibcpp::boxplot<Numeric>(const std::vector<Numeric>& data,const std::map<std::string, std::string> & keywords = {}); \
prefix bool matplotlibcpp::bar<Numeric>(const std::vector<Numeric> &               x,const std::vector<Numeric> &               y,std::string                                ec       = "black",std::string                                ls       = "-",double                                     lw       = 1.0,const std::map<std::string, std::string> & keywords = {}); \
prefix bool matplotlibcpp::bar<Numeric>(const std::vector<Numeric> &               y,std::string                                ec       = "black",std::string                                ls       = "-",double                                     lw       = 1.0,const std::map<std::string, std::string> & keywords = {}); \
prefix bool matplotlibcpp::barh<Numeric>(const std::vector<Numeric> &x, const std::vector<Numeric> &y, std::string ec = "black", std::string ls = "-", double lw = 1.0, const std::map<std::string, std::string> &keywords = { }); \
prefix bool matplotlibcpp::named_hist<Numeric>(std::string label,const std::vector<Numeric>& y, long bins=10, std::string color="b", double alpha=1.0); \
prefix bool matplotlibcpp::plot<Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& s = ""); \
prefix bool matplotlibcpp::contour<Numeric,Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y,const std::vector<Numeric>& z,const std::map<std::string, std::string>& keywords = {}); \
prefix bool matplotlibcpp::quiver<Numeric,Numeric,Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::vector<Numeric>& u, const std::vector<Numeric>& w, const std::map<std::string, std::string>& keywords = {}); \
prefix bool matplotlibcpp::quiver<Numeric,Numeric,Numeric,Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::vector<Numeric>& z, const std::vector<Numeric>& u, const std::vector<Numeric>& w, const std::vector<Numeric>& v, const std::map<std::string, std::string>& keywords = {}); \
prefix bool matplotlibcpp::stem<Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& s = ""); \
prefix bool matplotlibcpp::semilogx<Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& s = ""); \
prefix bool matplotlibcpp::semilogy<Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& s = ""); \
prefix bool matplotlibcpp::loglog<Numeric,Numeric>(const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& s = ""); \
prefix bool matplotlibcpp::errorbar<Numeric,Numeric>(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::vector<Numeric> &yerr, const std::map<std::string, std::string> &keywords = {}); \
prefix bool matplotlibcpp::named_plot<Numeric>(const std::string& name, const std::vector<Numeric>& y, const std::string& format = ""); \
prefix bool matplotlibcpp::named_plot<Numeric,Numeric>(const std::string& name, const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& format = ""); \
prefix bool matplotlibcpp::named_semilogx<Numeric,Numeric>(const std::string& name, const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& format = ""); \
prefix bool matplotlibcpp::named_semilogy<Numeric,Numeric>(const std::string& name, const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& format = ""); \
prefix bool matplotlibcpp::named_loglog<Numeric,Numeric>(const std::string& name, const std::vector<Numeric>& x, const std::vector<Numeric>& y, const std::string& format = ""); \
prefix bool matplotlibcpp::plot<Numeric>(const std::vector<Numeric>& y, const std::string& format = ""); \
prefix bool matplotlibcpp::plot<Numeric>(const std::vector<Numeric>& y, const std::map<std::string, std::string>& keywords); \
prefix bool matplotlibcpp::stem<Numeric>(const std::vector<Numeric>& y, const std::string& format = ""); \
prefix void matplotlibcpp::text<Numeric>(Numeric x, Numeric y, const std::string& s = ""); \
prefix void matplotlibcpp::set_aspect<Numeric>(Numeric ratio); \
prefix void matplotlibcpp::ylim<Numeric>(Numeric left, Numeric right); \
prefix void matplotlibcpp::xlim<Numeric>(Numeric left, Numeric right); \
prefix void matplotlibcpp::xticks<Numeric>(const std::vector<Numeric> &ticks, const std::vector<std::string> &labels = {}, const std::map<std::string, std::string>& keywords = {}); \
prefix void matplotlibcpp::xticks<Numeric>(const std::vector<Numeric> &ticks, const std::map<std::string, std::string>& keywords); \
prefix void matplotlibcpp::yticks<Numeric>(const std::vector<Numeric> &ticks, const std::vector<std::string> &labels = {}, const std::map<std::string, std::string>& keywords = {}); \
prefix void matplotlibcpp::yticks<Numeric>(const std::vector<Numeric> &ticks, const std::map<std::string, std::string>& keywords); \
prefix void matplotlibcpp::margins<Numeric>(Numeric margin); \
prefix void matplotlibcpp::margins<Numeric>(Numeric margin_x, Numeric margin_y); \
prefix void matplotlibcpp::pause<Numeric>(Numeric interval); \

#define MATPLOTLIBCPP_ALL(prefix) \
MATPLOTLIBCPP(prefix, double) \
MATPLOTLIBCPP(prefix, float) \
MATPLOTLIBCPP(prefix, int) \
MATPLOTLIBCPP(prefix, long) \

#ifndef NO_MATPLOTLIBCPP_TEMPLATE_SPECIALIZATIONS
MATPLOTLIBCPP_ALL(extern template)
#endif
